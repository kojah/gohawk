...
Use [heapmodel](heapmodel.md) for the heap summary's public contract: named
roots, slots, targets, effects, requirements, and truncation. A missing edge
is not proof of no alias when the relevant slot was truncated. Graph building
and substitution are still in `ssaflow` during the boundary migration; do
not treat the new contract package as a second heap inference engine.
...
- [internal/syncgraph](syncgraph.md)
