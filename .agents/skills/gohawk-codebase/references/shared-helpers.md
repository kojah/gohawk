...
The `ssaflow` and `lifecyclefacts` helpers, indexed by the question each one
answers, followed by the generated index of every exported helper.
...
[the fact model](../../../../docs/development/fact-model.md) for what a fact can express.
...
`internal/passes/lifecyclefacts`, with the synopsis of its doc comment.
