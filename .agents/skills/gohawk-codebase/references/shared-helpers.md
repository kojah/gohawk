...
| `SameValue`, `SameAsAny`, `ValueContainsValue` | possible identity or containment; not a guarantee that an action settles the target |
...
| `ReturnedValueOwnsValue`, `ReturnedSameAsAny` | does a return carry the value or its owner? |
...
| `StaticCalls`, `SourceSSAFunctions`, `FunctionFile` | call graphs and source functions |
...
| `SameValue` | ssaflow | SameValue reports a possible identity through conversions, any phi edge, and local storage history. |
