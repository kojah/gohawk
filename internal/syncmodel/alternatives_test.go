...
func TestExpandRequiresEveryAlternativeToBeComplete(t *testing.T) {
	...
	selection := concurrencyfacts.Summary{
		Reason: concurrencyfacts.ReasonSelectAlternatives, AlternativesComplete: true,
	...
	if !reason.Empty() || len(graphs) != 2 || !graphs[0].Complete() || !graphs[1].Complete() ||
	...
	if graphs, reason := Expand(selection); len(graphs) != 0 || reason.Empty() {
	...
	if graphs, reason := Expand(selection); len(graphs) != 0 || reason != graphFailure(ReasonAlternativeLimit) {
