...
func isParameter(_ ssaflow.ReachingWalk, value ssa.Value) bool {
...
func TestReachingWalkFoldsPhiEdges(t *testing.T) {
	...
	isSecond := func(_ ssaflow.ReachingWalk, value ssa.Value) bool { return value == second }

	if !ssaflow.NewReachingWalk(0).Any(returned, isSecond) {
		t.Fatal("Any should find the second parameter through the phi")
	}
	if ssaflow.NewReachingWalk(0).Every(returned, isSecond) {
		t.Fatal("Every should fail when only one edge is the second parameter")
	}
	if !ssaflow.NewReachingWalk(0).Every(returned, isParameter) {
...
func TestReachingWalkEveryOfNeedsValues(t *testing.T) {
	pkg := buildTestSSA(t, reachingSource)
	merged := pkg.Func("merge")
	always := func(ssaflow.ReachingWalk, ssa.Value) bool { return true }
	if ssaflow.NewReachingWalk(0).EveryOf(nil, always) {
		t.Fatal("no values must prove nothing")
	}
	if !ssaflow.NewReachingWalk(0).EveryOf([]ssa.Value{merged.Params[1], merged.Params[2]}, isParameter) {
...
func TestResolveReachingValueRequiresAgreement(t *testing.T) {
	pkg := buildTestSSA(t, reachingSource)
	typeName := func(_ ssaflow.ReachingWalk, value ssa.Value) (string, bool) {
	...
	resolved, ok := ssaflow.ResolveReachingValue(ssaflow.NewReachingWalk(0), returnedValue(t, pkg.Func("same")), typeName, identity)
	...
	if _, ok := ssaflow.ResolveReachingValue(ssaflow.NewReachingWalk(0), returned, typeName, identity); ok {
		t.Fatal("an opaque MakeInterface must leave the value unresolved")
	}
	if _, ok := ssaflow.ResolveReachingValue(ssaflow.NewReachingWalk(ssaflow.TransparentMakeInterface), returned, typeName, identity); !ok {
		t.Fatal("a transparent MakeInterface should expose agreeing parameters")
	}
	disagreeing := func(_ ssaflow.ReachingWalk, value ssa.Value) (string, bool) { return value.Name(), true }
	if _, ok := ssaflow.ResolveReachingValue(ssaflow.NewReachingWalk(0), returnedValue(t, pkg.Func("merge")), disagreeing, identity); ok {
...
func TestDeferredInstructionsAndClosureBindingPairs(t *testing.T) {
	pkg := buildTestSSA(t, reachingSource)
	defers := ssaflow.InstructionsOf[*ssa.Defer](pkg.Func("deferring"))
	if len(defers) != 2 {
	...
	pairs := ssaflow.ClosureBindingPairs(function, closure)
	if len(pairs) != 1 || pairs[0].Free != function.FreeVars[0] || pairs[0].Binding != closure.Bindings[0] {
		t.Fatalf("unexpected pairs %v", pairs)
	}
	if ssaflow.ClosureBindingPairs(function, nil) != nil || ssaflow.ClosureBindingPairs(nil, closure) != nil {
...
`)
	...
	for _, returned := range ssaflow.InstructionsOf[*ssa.Return](pkg.Func("open")) {
