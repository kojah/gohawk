		...
		for method, target := range map[string]*ParameterMask{
			...
			if ownsOnEveryReturn(function, parameter, func(instruction ssa.Instruction) bool {
				...
				if common != nil && ssautil.CallName(common) == method &&
					ssautil.ValueDerivesFrom(ssautil.CallReceiver(common), parameter, map[ssa.Value]bool{}) {
					return true
				}
				imported, ok := importFact(pass, instruction)
...
func returnedOwnerOnEveryReturn(function *ssa.Function, parameter ssa.Value) bool {
	...
	return !ssautil.UnownedReturnFromEntryAllow(function, func(ssa.Instruction) bool { return false }, func(returned *ssa.Return) bool {
...
func storedInReceiverOnEveryReturn(function *ssa.Function, receiver, parameter ssa.Value) bool {
	return !ssautil.UnownedReturnFromEntry(function, func(instruction ssa.Instruction) bool {
		store, ok := instruction.(*ssa.Store)
		if !ok || !ssautil.ValueDerivesFrom(store.Val, parameter, map[ssa.Value]bool{}) {
			return false
		}
		field, ok := store.Addr.(*ssa.FieldAddr)
		return ok && ssautil.ValueDerivesFrom(field.X, receiver, map[ssa.Value]bool{})
	})
}
