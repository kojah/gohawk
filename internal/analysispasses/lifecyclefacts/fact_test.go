...
`)
	...
	if !proof.Proven() || proof.Provenance != ssautil.EvidenceFromImportedFact || proof.Reason != reasonLifecycleSummary {
	...
	if rejected.State != ssautil.EvidenceDisproven || rejected.Provenance != ssautil.EvidenceFromImportedFact {
	...
	if unknown.State != ssautil.EvidenceUnknown || unknown.Reason != ssautil.EvidenceUnavailable {
...
func findLifecycleCall(t *testing.T, function *ssa.Function, name string) *ssa.Call {
	...
	for _, block := range function.Blocks {
		for _, instruction := range block.Instrs {
			...
			if ok && ssautil.CallName(call.Common()) == name {
				return call
			}
		}
	}
