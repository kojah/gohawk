...
`)
	...
	for _, block := range function.Blocks {
		channel, ok := SelectedReceiveChannel(block)
		if !ok {
			continue
		}
	...
	if _, ok := SelectedReceiveChannel(matched); ok {
		t.Fatal("shared successor was treated as a selected receive")
	}
	if channel, ok := SelectedReceiveOnEdge(matched.Preds[0], matched); !ok || channel != function.Params[0] {
	...
	for _, name := range []string{"ordinary", "stringComparison", "boolComparison"} {
		for _, block := range pkg.Func(name).Blocks {
			if _, ok := SelectedReceiveChannel(block); ok {
				t.Fatalf("%s branch was treated as a selected receive", name)
			}
		}
	}
	if _, ok := SelectedReceiveChannel(nil); ok {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			start := findSSAInstruction(t, function, func(instruction ssa.Instruction) bool {
				return CallName(InstructionCall(instruction)) == "start"
			})
			exact := func(instruction ssa.Instruction) bool { return CallName(InstructionCall(instruction)) == "cleanup" }
			uncertain := func(instruction ssa.Instruction) bool {
				return exact(instruction) || CallName(InstructionCall(instruction)) == "opaque"
			}
			edge := func(from, to *ssa.BasicBlock) bool {
				channel, selected := SelectedReceiveOnEdge(from, to)
				return selected && channel == function.Params[0]
			}
			if got := UnownedReturnWithEdges(start, exact, nil, edge); got != test.unowned {
				t.Errorf("after start = %v, want %v", got, test.unowned)
			}
			if got := UnownedReturnFromEntryWithEdges(function, exact, edge); got != test.unowned {
				t.Errorf("from entry = %v, want %v", got, test.unowned)
			}
			if got := UnownedReturnAssumingNonNilWithEdges(start, function.Params[0], uncertain, nil, edge); got != test.uncertainUnowned {
				t.Errorf("uncertain/non-nil = %v, want %v", got, test.uncertainUnowned)
			}
			if !UnownedReturn(start, uncertain, nil) {
