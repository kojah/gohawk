...
func sameCallCycle(function, callee *ssa.Function) bool {
	...
	if callee == function || ResolvedFunction(callee) == ResolvedFunction(function) {
		return true
	}
	return reachableCallees(callee)[function] || reachableCallees(ResolvedFunction(callee))[ResolvedFunction(function)]
}
...
func staticCallees(function *ssa.Function) []*ssa.Function {
	...
	for _, block := range function.Blocks {
		for _, instruction := range block.Instrs {
			...
			if resolved := ResolvedFunction(common.StaticCallee()); resolved != common.StaticCallee() {
