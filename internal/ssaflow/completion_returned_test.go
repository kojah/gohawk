	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if len(function.Params) != 0 {
				...
				target = InstructionsOf[*ssa.Extract](function)[0]
			}
			var invocation ssa.Instruction
			if deferred := InstructionsOf[*ssa.Defer](function); len(deferred) != 0 {
				invocation = deferred[0]
			} else {
				calls := InstructionsOf[*ssa.Call](function)
				invocation = calls[len(calls)-1]
			}
			request := CompletionRequest{Instruction: invocation, Target: target, Methods: []string{"Close"}, Budget: NewSearchBudget(2000)}
