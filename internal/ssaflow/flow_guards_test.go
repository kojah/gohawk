...
`)
	...
	guards, contradiction := PathGuards(nil).Extend(branches[0], branches[0].Succs[0], nil)
	if contradiction != GuardConsistent || len(guards) != 1 || !guards[0].Stable || !guards[0].Value {
		t.Fatalf("first branch: %+v %v", guards, contradiction)
	}
	last := branches[len(branches)-1]
	if _, contradiction := guards.Extend(last, last.Succs[1], nil); contradiction != GuardStableContradiction {
		t.Errorf("the other arm of a stable guard should contradict, got %v", contradiction)
	}
	if _, contradiction := guards.Extend(last, last.Succs[0], nil); contradiction != GuardConsistent {
		t.Errorf("the same arm of a stable guard is consistent, got %v", contradiction)
	}
	if _, contradiction := guards.Extend(branches[1], branches[1].Succs[1], nil); contradiction != GuardConsistent {
		t.Errorf("an unrelated branch is consistent, got %v", contradiction)
	}
	keepLoaded := func(guard PathGuard) bool { return !guard.Stable }
	if kept, _ := PathGuards(nil).Extend(branches[0], branches[0].Succs[0], keepLoaded); len(kept) != 0 {
		t.Errorf("a filtered-out guard must not be remembered: %+v", kept)
	}

	loadedFn := pkg.Func("loaded")
	branches = branchBlocks(loadedFn)
	guards, _ = PathGuards(nil).Extend(branches[0], branches[0].Succs[0], nil)
	if len(guards) != 1 || guards[0].Stable {
		t.Fatalf("a field load is a loaded guard: %+v", guards)
	}
	if _, contradiction := guards.Extend(branches[1], branches[1].Succs[1], nil); contradiction != GuardLoadedContradiction {
		t.Errorf("the other arm of a loaded guard is uncertain, got %v", contradiction)
	}
	for _, store := range InstructionsOf[*ssa.Store](loadedFn) {
