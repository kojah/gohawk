	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			for _, call := range InstructionsOf[*ssa.Call](fn) {
				if CallName(call.Common()) == "Use" {
					receiver = CallReceiver(call.Common())
				}
			}
			proof := ProveEnclosingCompletion(EnclosingCompletionRequest{
				Function: fn, Value: receiver, Methods: []string{"Close"}, Budget: NewSearchBudget(10000),
			})
			...
			limited := ProveEnclosingCompletion(EnclosingCompletionRequest{
				Function: fn, Value: receiver, Methods: []string{"Close"}, Budget: NewSearchBudget(1),
			})
			if limited.State != EvidenceUnknown || limited.Reason != EvidenceBudgetExhausted {
