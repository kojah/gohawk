...
func runAPIShape(pass *analysis.Pass, config apiShapeConfig) (any, error) {
	...
	for _, file := range pass.Files {
		...
		ast.Inspect(file, func(node ast.Node) bool {
			...
			if config.maxParameters > 0 && len(parameters) > config.maxParameters {
				reportf(pass, checkAPIParameterCount, declaration.Name.Pos(), "exported API has %d parameters; use an Input or config struct", len(parameters))
			}
	...
	for name, forms := range receivers {
		if forms == 3 {
			reportf(pass, checkAPIMixedReceivers, receiverPositions[name], "type %s mixes pointer and value receivers", name)
		}
	}
	return nil, nil
}
...
func reportAdjacentParameters(pass *analysis.Pass, position token.Pos, parameters []types.Type, config apiShapeConfig) {
	for start := 0; start < len(parameters); {
		...
		if config.maxAdjacentSameType > 0 && end-start > config.maxAdjacentSameType {
			reportf(pass, checkAPIAdjacentSameType, position, "%d adjacent parameters share type %s; use an Input struct", end-start, types.TypeString(parameters[start], nil))
		} else if end-start >= 2 && optionalScalar(parameters[start]) {
			reportf(pass, checkAPIAdjacentOptional, position, "adjacent optional scalar parameters are easy to swap; use an Input struct")
		}
		start = end
	}
}
