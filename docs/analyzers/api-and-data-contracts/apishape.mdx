...
| --- | --- | --- | --- |
| `apishape/parameter-count` | Reports exported APIs with more than the configured maximum number of parameters. | <CheckProfile profile="default" /> | <CheckTags tags={["policy"]} /> |
| `apishape/mixed-receivers` | Reports types that mix pointer and value receiver methods. | <CheckProfile profile="default" /> | <CheckTags tags={["policy"]} /> |
| `apishape/adjacent-same-type` | Reports long adjacent runs of parameters that share one type. | <CheckProfile profile="default" /> | <CheckTags tags={["policy"]} /> |
| `apishape/adjacent-optional-scalars` | Reports adjacent optional scalar parameters that are easy to swap. | <CheckProfile profile="default" /> | <CheckTags tags={["policy"]} /> |
