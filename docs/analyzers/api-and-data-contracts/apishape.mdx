...
swap. Signatures constrained by an implemented interface, an observed callback
use, or explicit blank compatibility parameters are accepted. It also reports
types that mix pointer and value receiver methods.
...
| --- | --- | --- | --- |
| <CheckIdentity name="parameter-count" tier="extended" /> | policy | extended | Reports unconstrained exported APIs with more than the configured maximum number of parameters. |
| <CheckIdentity name="mixed-receivers" tier="extended" /> | policy | extended | Reports types that mix pointer and value receiver methods. |
| <CheckIdentity name="adjacent-same-type" tier="extended" /> | policy | extended | Reports long adjacent runs of parameters in unconstrained signatures that share one type. |
| <CheckIdentity name="adjacent-optional-scalars" tier="extended" /> | policy | extended | Reports adjacent optional scalar parameters in unconstrained signatures that are easy to swap. |
