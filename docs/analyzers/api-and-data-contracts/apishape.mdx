...
| --- | --- |
| <CheckIdentity name="parameter-count" optIn /> | Reports unconstrained exported APIs with more than the configured maximum number of parameters. |
| <CheckIdentity name="mixed-receivers" optIn /> | Reports types that mix pointer and value receiver methods. |
| <CheckIdentity name="adjacent-same-type" optIn /> | Reports long adjacent runs of parameters in unconstrained signatures that share one type. |
| <CheckIdentity name="adjacent-optional-scalars" optIn /> | Reports adjacent optional scalar parameters in unconstrained signatures that are easy to swap. |
