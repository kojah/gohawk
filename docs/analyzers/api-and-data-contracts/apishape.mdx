...
| --- | --- |
| <CheckIdentity name="parameter-count" tags={["policy"]} /> | Reports exported APIs with more than the configured maximum number of parameters. |
| <CheckIdentity name="mixed-receivers" tags={["policy"]} /> | Reports types that mix pointer and value receiver methods. |
| <CheckIdentity name="adjacent-same-type" tags={["policy"]} /> | Reports long adjacent runs of parameters that share one type. |
| <CheckIdentity name="adjacent-optional-scalars" tags={["policy"]} /> | Reports adjacent optional scalar parameters that are easy to swap. |
