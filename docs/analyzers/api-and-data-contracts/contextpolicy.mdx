...
| --- | --- |
| <CheckIdentity name="context-first" tags={["reliability","policy"]} /> | Reports context.Context parameters that are not first. |
| <CheckIdentity name="context-storage" tags={["reliability","policy"]} /> | Reports context.Context values stored in structs. |
| <CheckIdentity name="test-context" optIn tags={["policy"]} /> | Reports tests that use context.Background instead of the testing handle's context. |
| <CheckIdentity name="nil-context" tags={["correctness"]} /> | Reports definitely nil context.Context arguments. |
