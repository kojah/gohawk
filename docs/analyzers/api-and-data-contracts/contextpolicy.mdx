...
Reports functions whose first `context.Context` parameter follows another
parameter, contexts stored in structs, and calls that pass a definitely nil
context. In supported test code, it also prefers `t.Context()` or `b.Context()`
...
| --- | --- |
| <CheckIdentity name="context-first" /> | Reports functions whose first context.Context parameter follows another parameter. |
| <CheckIdentity name="context-storage" /> | Reports context.Context values stored in structs. |
| <CheckIdentity name="test-context" optIn /> | Reports tests that use context.Background instead of the testing handle's context. |
