...
Reports channels in production files with large constant capacities that have
no nearby rationale, code that closes a channel received from its caller, and
sends that can happen after the channel has been closed. Test fixture capacity
is accepted, while the closing-ownership checks continue to analyze tests.
...
| --- | --- |
| <CheckIdentity name="capacity-rationale" optIn /> | Reports large constant channel capacities in production files without a nearby bounded rationale. |
