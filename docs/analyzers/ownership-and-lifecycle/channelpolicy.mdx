...
| Check (`channelpolicy/…`) | What it detects | Profile | Tags |
| --- | --- | --- | --- |
| `capacity-rationale` | Reports large constant channel capacities without a nearby bounded rationale. | <CheckProfile profile="opt-in" /> | <CheckTags tags={["policy"]} /> |
| `caller-close` | Reports functions that close channels received from their callers. | <CheckProfile profile="default" /> | <CheckTags tags={["reliability","policy"]} /> |
| `send-after-close` | Reports sends reachable after a channel has been closed. | <CheckProfile profile="default" /> | <CheckTags tags={["correctness"]} /> |
