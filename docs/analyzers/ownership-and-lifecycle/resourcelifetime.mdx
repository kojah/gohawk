...
| <CheckIdentity name="use-after-release" tier="experimental" /> | hazard | experimental | Reports an invalidating operation on a resource that a direct release dominates. |
...
The experimental `use-after-release` check is the dual of the leak check. After a
