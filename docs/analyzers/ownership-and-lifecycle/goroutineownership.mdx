...
| --- | --- | --- | --- |
| `goroutineownership/unjoined` | Reports goroutines with a recognizable join or lifecycle mechanism that is not honored on every return path. | <CheckProfile profile="default" /> | <CheckTags tags={["reliability"]} /> |
| `goroutineownership/detached` | Reports goroutines without a recognizable join handle or lifecycle owner. | <CheckProfile profile="opt-in" /> | <CheckTags tags={["reliability"]} /> |
| `goroutineownership/abandoned-send` | Reports producer goroutines that can block after their receiver stops waiting. | <CheckProfile profile="default" /> | <CheckTags tags={["reliability"]} /> |
