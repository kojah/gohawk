...
| --- | --- |
| <CheckIdentity name="unjoined" profile="default" tags={["reliability"]} /> | Reports goroutines with a recognizable join or lifecycle mechanism that is not honored on every return path. |
| <CheckIdentity name="detached" profile="opt-in" tags={["reliability"]} /> | Reports goroutines without a recognizable join handle or lifecycle owner. |
| <CheckIdentity name="abandoned-send" profile="default" tags={["reliability"]} /> | Reports producer goroutines that can block after their receiver stops waiting. |
