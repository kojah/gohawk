...
Fresh acquisition inference requires a field containing a known concrete
resource type. A custom `Close` method alone does not establish acquisition:
the result may be a lazy or empty handle. Nested custom owners and
interface-only resource results remain unknown, so leaks through those
constructors may be missed.
