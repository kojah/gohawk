...
The check follows values derived from the range into ordered accumulators. It
does not report iteration used only for membership, independently processed
items, or a map proven to contain exactly one entry, and it recognizes sorting
performed on every path before the accumulated value is observed.
...
| --- | --- | --- | --- |
| <CheckIdentity name="map-output-order" tier="extended" /> | hazard | extended | Reports map iteration that reaches ordered output without explicit sorting. |
