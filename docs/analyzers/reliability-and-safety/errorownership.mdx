...
| --- | --- |
| <CheckIdentity name="log-and-return" optIn tags={["reliability","policy"]} /> | Reports functions that both log and return the same error. |
| <CheckIdentity name="text-classification" tags={["reliability"]} /> | Reports production code that classifies errors by matching their text. |
| <CheckIdentity name="mismatched-inline-error" tags={["correctness"]} /> | Reports inline error declarations whose condition checks a different error. |
