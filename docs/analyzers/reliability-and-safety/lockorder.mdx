...
| --- | --- | --- |
| <CheckIdentity name="missing-release" /> | defect | Reports return paths that leave an owned lock held. |
| <CheckIdentity name="recursive-acquire" /> | defect | Reports attempts to acquire a lock that is already held. |
| <CheckIdentity name="contradictory-order" optIn /> | hazard | Reports inconsistent acquisition order for the same pair of locks. |
