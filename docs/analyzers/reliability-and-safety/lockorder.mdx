...
| --- | --- |
| <CheckIdentity name="missing-release" /> | Reports return paths that leave an owned lock held. |
| <CheckIdentity name="recursive-acquire" /> | Reports attempts to acquire a lock that is already held. |
| <CheckIdentity name="contradictory-order" /> | Reports inconsistent acquisition order for the same pair of locks. |
