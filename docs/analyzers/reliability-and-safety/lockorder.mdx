...
| --- | --- | --- | --- |
| `lockorder/missing-release` | Reports return paths that leave an owned lock held. | <CheckProfile profile="default" /> | <CheckTags tags={["correctness"]} /> |
| `lockorder/recursive-acquire` | Reports attempts to acquire a lock that is already held. | <CheckProfile profile="default" /> | <CheckTags tags={["correctness"]} /> |
| `lockorder/contradictory-order` | Reports inconsistent acquisition order for the same pair of locks. | <CheckProfile profile="default" /> | <CheckTags tags={["correctness"]} /> |
