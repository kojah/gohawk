...
| --- | --- |
| <CheckIdentity name="missing-release" tags={["correctness"]} /> | Reports return paths that leave an owned lock held. |
| <CheckIdentity name="recursive-acquire" tags={["correctness"]} /> | Reports attempts to acquire a lock that is already held. |
| <CheckIdentity name="contradictory-order" tags={["correctness"]} /> | Reports inconsistent acquisition order for the same pair of locks. |
