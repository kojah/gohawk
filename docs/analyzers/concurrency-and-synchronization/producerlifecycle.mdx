...
- The send is a plain send in an exported function or method. It is not in a
  `select` with another way out, not inside the loop itself, not in the
  function that starts the loop, and not behind a branch on the owner's own
  state, such as a `running` flag.
