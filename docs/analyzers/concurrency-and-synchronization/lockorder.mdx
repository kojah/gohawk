...
A worker pool with an unknown count, such as `for range items { wg.Add(1); go
worker() }`, is read as one representative child: every copy needs the same
held mutex, so no copy can release another, and one blocked child proves the
cycle for any number of them. The loop must be a range or counted loop, its
every-iteration body may only add to the group and launch children, and the
children must use the same resources on every iteration. Registration can
interleave with launches under the held lock. Counter captures,
per-iteration resources, and unsupported control flow remain unknown. A
different count, another decrement, early `Done`, unknown participant, or
