...
dependency: a function holds a fresh local `sync.Mutex` while receiving from a
fresh completion channel, and every child that could close it must acquire
that exact mutex first. The receive cannot finish until a signal;
...
more than four launches, divergent helper effects, an opaque participant, or a
...
participant, or a helper with divergent effects is inconclusive. A later close
...
locks and before it waits, and complete straight-line child effects. A dynamic
