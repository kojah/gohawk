...
| <CheckIdentity name="lock-and-join" tier="experimental" /> | defect | experimental | Reports a wait while holding the exact local mutex that the sole completion worker must acquire. |
| <CheckIdentity name="channel-lock-cycle" tier="experimental" /> | defect | experimental | Reports a receive on a fresh unbuffered channel whose sole sender first needs the receiver's held mutex. |
...
fresh completion channel, and its sole worker must acquire that exact mutex
before it can close the channel. The receive cannot finish until the close;
...
a second launch, divergent helper effects, an opaque participant, or a budget
cut is inconclusive. It does not attempt general cycles, WaitGroups, buffered
...
requires that the worker's first synchronization event acquire the exact fresh
mutex held by the receiver, and that its first signal on the waited-for
channel be the send. A buffered or unknown-capacity channel, an outside
