...
| <CheckIdentity name="lock-and-join" tier="experimental" /> | defect | experimental | Reports a wait while holding the local mutex that every possible completion worker must acquire. |
...
dependency: a function holds a fresh local `sync.Mutex` or `sync.RWMutex` while receiving from a
...
operations from an incomplete summary. A mutex or channel supplied by a caller,
...
requires that every possible sender first acquire the exact fresh mutex held
...
count per known child, holds a fresh local mutex while calling `Wait`, and
unlocks only afterward. Each child must acquire that exact mutex before its
