...
the flag value from a different predecessor. Up to four exact Boolean phi
constants are carried through later blocks and refreshed at each merge; unknown
incoming values discard the old binding. This does not assume mutable fields
stay unchanged or solve arbitrary conditional lock protocols. Up to eight
stable comparisons of parameters and constants can also be retained across
