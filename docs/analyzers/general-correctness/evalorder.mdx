---
title: evalorder
seoTitle: "evalorder: Detect fragile evaluation-order dependencies in Go"
...
| --- | --- | --- | --- |
| <CheckIdentity name="operand-mutation" tier="core" /> | hazard | core | Reports expressions whose later operand mutates a value read by an earlier operand. |
...
JSON decoding into a fresh, otherwise unused plain map is not enough to report
an earlier whole-map snapshot: object decoding updates entries shared by both
values. The check declines this uncertain case without proving the input is an
object; it can therefore miss a header reset by JSON `null`. Earlier entry reads,
nil or reassigned maps, custom decoders, and explicit map-replacing helpers remain
checked.
