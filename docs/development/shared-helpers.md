...
Consumers go through `LifecycleEvidence`. Raw `ImportObjectFact` and
`ExportObjectFact` calls belong only in the package that defines the fact
type; `TestObjectFactsStayInTheirDefiningPackage` enforces it. See
...
Keep the doc comment as the contract, and then add the row here.
